...
router.post('/', async ( req, res, next) => {
    try{
        ...
        const resp = await db.query(`INSERT INTO companies (code,name,description) VALUES ($1, $2, $3) RETURNING *`, [codeSlug,name,description]);
...
router.delete('/:code', async ( req, res, next) => {
    try{
        const resp = await db.query(`DELETE FROM companies WHERE code = $1`, [req.params.code]);
        if(!resp){
            throw new ExpressError(`${req.params.code} doesnt exist`, 404);
        };
